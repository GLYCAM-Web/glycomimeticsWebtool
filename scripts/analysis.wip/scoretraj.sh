...
# for i in analog_OTP93_3_0; do
    moiety_name=$(echo ${i} | sed 's/analog_//')
    echo "moiety name: ${moiety_name}"

    $(dirname "$(readlink -f "${BASH_SOURCE[0]}")")/../../../programs_src/MGLTools-1.5.4/MGLToolsPckgs/AutoDockTools/Utilities24/prepare_receptor4.py -r ../${i}/1_leap/cocomplex_nowat_noion.pdb -A None -U None -o cocomplex_pdbqts/cocomplex_${i}.pdbqt
...
for i in *.csv;do
    x=$(echo ${i} | sed 's/.csv//')
    tail -n 1 ${i} | awk -v moiety="${x}" '{print moiety" "$2"\t"$3}' >> VINA_summary.txt
done

echo ""
cat VINA_summary.txt
