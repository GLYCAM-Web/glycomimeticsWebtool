...
#########################################################################
#
# $Header: /opt/cvs/python/packages/share1.5/NetworkEditor/ports.py,v 1.177 2008/10/10 21:14:24 sanner Exp $
#
# $Id: ports.py,v 1.177 2008/10/10 21:14:24 sanner Exp $
#
...
class NGWidget:
    ...
    def configure(self, **kw):
        print 'WARNING running without GUI: NGWidget.configure() not implemented'
        #print kw
...
"""
    ...
    def getDataStr(self, maxLen = None):
        ...
        if type(self.data)==types.InstanceType:
            header = 'Type:%s'%self.data.__class__
        else:
            header = 'Type:%s'%type(self.data)
        ...
        if isinstance(self.data, Numeric.ArrayType):
            ...
            if type(self.data) == 'str':
                ...
                lstrdata = str(self.data)
                header += ', length:' + str(len(lstrdata))
                length = len(lstrdata)
        header += '\n'
...
class InputPort(Port):
    ...
    def __init__(self, name, node, datatype=None, required=True,
        ...
        # _newdata is set to True (since it used to provide data and now has
        # None)
        self._newdata = False
...
##                     self.node.showInNodeWidgets()
    ...
    def unbindWidget(self, force=False):
        #print "unbindWidget"
        if self.node.getEditor() is None: return
        inNode = False
        if self.widget:
        ...
        if wasExpanded and len(self.node.widgetDescr.keys()):# repack node
            ...
            for v in self.node.widgetDescr.values():
                if v.has_key('master') and v['master'] in ['node', 'Node']:
                    resize = False
                    break
            if resize is True: # only resize, if we have no node widget left
...
"""
        #print "getData", self
        #if self.node.name == 'lala' and self.name == 'in1':
        #if self.name == 'nodes':
        #    import pdb;pdb.set_trace()
