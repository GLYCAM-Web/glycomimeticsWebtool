...
########################################################################
#
# $Header: /opt/cvs/Volume/VisionInterface/VolumeNodes.py,v 1.167.2.1 2008/11/13 21:38:13 annao Exp $
#
# $Id: VolumeNodes.py,v 1.167.2.1 2008/11/13 21:38:13 annao Exp $
#
...
Output Ports
    data: coordinates of the voxel centers
"""
    ...
    def __init__(self, name='GridPoints', **kw):
        ...
        self.widgetDescr['dim'] = {
            ...
            'oneTurn':10}
        self.inputPortsDescr.append(datatype='int', name='dim')
	self.outputPortsDescr.append(datatype='NumericArray', name='data')

	code = """def doit(self, dim):
    import numpy.oldnumeric as Numeric
    coords= Numeric.zeros( (dim*dim*dim, 3), 'f')
    dim2 = dim*dim
    for i in xrange(dim):
        for j in xrange(dim):
            for k in xrange(dim):
                coords[i*dim2+j*dim+k] = (i, j, k)
    self.outputData(data=coords)
"""
...
"""
    ...
    def __init__(self, name='GridPoints', **kw):
        ...
        ip.append(datatype='list', name='origin')
        ip.append(datatype='list', name='stepsize')
	...
	code = """def doit(self, dimx, dimy, dimz, dtype, origin, stepsize):
...
"""
	...
	code = """def doit(self, array, headerGrid=None):
...
It is assumed the grids are of same shape.
...
"""
    ...
    if copy is False:
        ...
        newgrid = grid1.__class__( newdata, grid1.origin, grid1.stepSize[::-1],
                                   grid1.header)
...
name=None, geoms=None, instanceMatrices=None, geomOptions=None, parent=None):
    ...
    if self.selectedGeomIndex is not None:
        ...
        if self.inputPortByName['grid'].hasNewData():
...
##         code = """def doit(self, grid, xaxis, yaxis, zaxis, slicex, slicey, slicez, colorMap, name=None, instanceMatrices=None, geomOptions=None, parent=None):
...
Output Ports
    grid: 3D Box displaying the boundaries of the Volume
"""
...
'*.omap *.brix* *.dsn6* *.cns *.xplo*r* *.d*e*l*phi *.mrc *.rawiv *.spi *.uhbd'),
	...
	code = """def doit(self, filename, normalize=True):
        global ifd
...
"""
    ...
    def __init__(self, name='Read CCP4', **kw):
        ...
        self.inputPortsDescr.append(datatype='string', name='filename')
        self.inputPortsDescr.append(datatype='boolean', required=False, name='normalize')
...
"""
	...
	code = """def doit(self, grid, filename='output.rawiv'):
        if grid:
...
Output Ports
   ...
   """
    ...
    def __init__(self, name='UT-Isocontour', **kw):
        ...
        #self.readOnly = 1
        code = """def doit(self, grid3D, isovalue=None, calculatesignatures=None, verbosity=None):
...
"""
	...
	code = """def doit(self, grid, datatype=None, src_min=None, src_max=None, dst_min=None, dst_max=None, mapping=None, powerOf2=None):
    kw = {}
...
"""
    ...
    def __init__(self, constrkw={}, name='FillMapWithBox', library=None, **kw):
        ...
        code = """def doit(self, grid3D, Box, fillValue=None):
    bc1 = [0,0,0]
    bc2 = [0,0,0]
...
"""
    ...
    if grid3D.crystal:
        ctf = grid3D.crystal.toFractional
        # transfrorm sphere centers into fractional space
        centers = ctf(centers)
        # find radius of sphere containing max radius
        radii = map( lambda x, f=ctf: max( f((x, 0, 0))[0], f((0, x, 0))[1],
                                           f((0, 0, x))[2] ), radii)
        from mglutil.math.crystal import Crystal
        crystal = Crystal(grid3D.crystal.length, grid3D.crystal.angles)
    else:
    ...
    dcpk = discreteSpheres(centers, radii, grid3D.stepSize, grid3D.origin,
                           grid3D.dimensions)
    from Volume.Grid3D import Grid3DUC
    h = grid3D.header.copy()
    if h.has_key('nx'):
...
"""
    ...
    def __init__(self, constrkw={},  name='Gaussian Mask', library=None,**kw):
        ...
        ip.append(datatype='int', required=False, name='box')
        ip.append(datatype='float', required=False, name='apix')
        ip.append(datatype='float', required=False, name='res')
        ip.append(datatype='boolean', required=False, name='solv')
        ...
        code = """def doit(self, atoms, box=0, apix=1.0, res=1.5, solv=False):
...
"""
    ...
    def __init__(self, constrkw={},  name='UT-BlurSpheres(1)', library=None,**kw):
        ...
        ip.append(datatype='float', name='resY', required=False)
        ip.append(datatype='float', name='resZ', required=False)
        self.inputPortsDescr.append(datatype='boolean', name='switch')
        ...
        code = """def doit(self, coords, radius, blobbyness, oneRes, resX, resY=0, resZ=0, switch=0, dims=None):
...
class UTMesher(NetworkNode):
    ...
    def __init__(self, name='UT-mesh', **kw):
        ...
        ip.append(datatype='int', name='outersurf')
        ip.append(datatype='int', name='crossection')
        ip.append(datatype='int', name='planeX')
        ip.append(datatype='int', name='planeZ')
        ...
        code= """def doit(self, grid3DF,  meshtype, outer_isoval, outer_error, inner_isoval=None, inner_error=None, outersurf = 0, crossection = 0, planeX = 0, planeZ = 0):
        update = 0
...
vollib.addNode(UT_Iso, 'UTiso', 'Macros')
try:
    from QSlimLib import qslimlib
    vollib.addNode(UT_IsoDecim, 'UTiso+Decim', 'Macros')
except:
    pass
try:
    import MolKit
    vollib.addNode(CoarseMolSurf, 'CoarseMolSurf', 'Macros')
    #from CoarseMolSurf import CoarseMolSurf0
    #vollib.addNode(CoarseMolSurf0, 'CoarseMolSurf0', 'Macros')
except:
    pass


#vollib.addNode(GridPoints, 'GridPoints', 'Test')
...
vollib.addNode(TransposeGrid3D, 'Transpose', 'Operators')
vollib.addNode(Sample, 'sample', 'Operators')
vollib.addNode(SetOrigin, 'set origin', 'Operators')
vollib.addNode(SetStepSize, 'set stepSize', 'Operators')
vollib.addNode(Center, 'center', 'Operators')
vollib.addNode(LogicOP, 'logicOP', 'Operators')
vollib.addNode(ThresholdMask, 'ThresholdMask', 'Operators')
vollib.addNode(TriInterp, 'triInterp', 'Operators')
vollib.addNode(ArrayScalarOp, 'ArrayScalarOp', 'Operators')
vollib.addNode(ArrayArrayOp, 'ArrayArrayOp', 'Operators')
if foundGeometryNode:
    vollib.addNode(OrthoSlice, 'OrthoSlice', 'Operators')
vollib.addNode(Grid3DTo2D, 'Grid3DTo2D', 'Operators')
vollib.addNode(SetValuesAtPositions, 'SetAtPos', 'Operators')
