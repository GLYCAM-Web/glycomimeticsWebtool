...
# $Header: /opt/cvs/python/packages/share1.5/ViewerFramework/VFGUI.py,v 1.155.2.2 2009/06/05 21:54:09 sargis Exp $
#
# $Id: VFGUI.py,v 1.155.2.2 2009/06/05 21:54:09 sargis Exp $
#
    ...
    def softquit_cb(self):
        ...
        if hasattr(self.vf, 'logAllFile'):
            ...
            for fname in dirList:
                if fname.startswith('mvAll') \
                  and ( fname.endswith('.log.py') or fname.endswith('.log.py~') ) \
                  and self.vf.logAllFile.name.endswith(fname) is False: #so we keep the current session file
        ...
        if isinstance(self.ROOT, Tkinter.Tk) or \
           isinstance(self.ROOT, Tkinter.Toplevel):
            ...
            if self.ROOT.winfo_screenwidth() < 1280:
              self.ROOT.option_add('*font',(ensureFontCase('helvetica'), 10, 'normal'))
            else:
              self.ROOT.option_add('*font',(ensureFontCase('helvetica'), 14, 'normal'))
...
##         self.belowCamPane = self.pw.add('below', min = .1, max = .1)
        ...
        self.vwrCanvasDocked = Tkinter.Frame(self.ROOT, width=300, height=200)
        self.vwrCanvasDocked.pack(anchor='n', expand=1, fill='both')
        self.vwrCanvasDocked.forget()
        
        self.vwrCanvasFloating = Tkinter.Toplevel(
...
"""
    ...
    def rebuildCamera(self, stereo=None):
        ...
        if stereo is None:
            stereo = (cam.stereoMode == 'STEREO_BUFFERS')
        ...
        vi.removeAllTheDisplayListsExceptTheTemplates()
        ...
        # Delete the currentCamera camera
        vi.DeleteCamera(vi.cameras[0])
    ...
    def floatCamera(self, stereo=None):
        #print "floatCamera"
        if self.vwrCanvasFloating.winfo_ismapped():
            return
        else:
        ...
        lNewCam = vi.AddCamera(master=self.vwrCanvasFloating, stereo=stereo)
        ...
        apply(cam.fog.Set,(), fogState)
        vi.startAutoRedraw()
        self.vwrCanvasFloating.deiconify()
...
##              self.pendingLog = []
    ...
    def addMenuCommand(self, menu, name, cb, index, kw={}):
    ...
    def addCommandMenuGUI(self, cmdGUI, cmdcb):
        ...
        if gui[5]['menuEntryType']=='command':
            self.addMenuCommand( menu, gui[5]['menuEntryLabel'], cb,
                                 gui[5]['index'], gui[4] )
            ...
            gui[4]['command'] = cb
            apply( menu.add_checkbutton, (), gui[4])
    ...
    def askFileOpen(self, master, idir = None, ifile=None, types=None,
                    title='open'):
        file = fileOpenAsk(master, idir, ifile, types,title)
        return file
...
def fileOpenAsk(master, idir=None, ifile=None, types=None,
    ...
    file = tkFileDialog.askopenfilename( parent = master,
                                         filetypes=types,
                                         initialdir=idir,
                                         initialfile=ifile,
                                         title=title)
