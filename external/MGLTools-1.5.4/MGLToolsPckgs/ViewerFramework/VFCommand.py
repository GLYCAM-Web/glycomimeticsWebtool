...
# $Header: /opt/cvs/python/packages/share1.5/ViewerFramework/VFCommand.py,v 1.74.2.1 2009/03/11 00:22:01 sargis Exp $
#
# $Id: VFCommand.py,v 1.74.2.1 2009/03/11 00:22:01 sargis Exp $
#
...
picked objects.
    ...
    def setCommands(self, command, modifier=None):
        if command:
    ...
    def execCmd(self, objects):
        ...
        if cmd:
            ...
            apply(cmd, (objects,), defaultValues)
            #cmd(objects)
...
by afterDoit if the command is successfully executed"""
        ...
        s = ''
        for l in before: s = s + l + '\n'
        s = s + 'self.' + name + argString
        self.undoCmds = self.undoCmds + s + '\n'
    ...
    def beforeDoit(self, args, kw, log, setupUndo, busyIdle):
    ...
    def doitWrapper(self, *args, **kw):
        ...
        self.beforeDoit(args, kw, log, setupUndo, busyIdle)
...
##         args, kw = self.getLogArgs( args, kw )
    ...
    def showForm(self, formName='default', force=0, master=None, root=None,
                 ...
                 okcancel=1, onDestroy=None, help=None, posx=None, posy=None):
        """
