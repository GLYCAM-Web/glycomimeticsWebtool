...
#########################################################################
#
# $Header: /opt/cvs/python/packages/share1.5/Vision/VPE.py,v 1.204.2.1 2009/04/22 23:47:10 vareille Exp $
#
# $Id: VPE.py,v 1.204.2.1 2009/04/22 23:47:10 vareille Exp $
#
...
class VisualProgramingEnvironment(NetworkBuilder, NoGuiVPE):
    ...
    def __init__(self, name='NetworkBuilder', master=None, font=None, resourceFolder='mgltools',
                 withShell=1, width=600, height=600, **kw):
        ...
        if sys.platform == 'win32':
            ...
            modulePagesPane = self.top.add('ModulePages', min=150, size=150)
            #self.top.configurepane('ModulePages',min=100, size=150 )
        else: 
            modulePagesPane = self.top.add('ModulePages', min=100, size=150)
        ...
        # add notbook for module libraries
...
##          self.modSearch.component('label').configure(image=findicon)
        ...
        apply( NetworkBuilder.__init__, (self, name, ed, font, withShell, resourceFolder),
               kw )
        ...
        ensureDefaultUserLibFile(resourceFolder=resourceFolder)
        from Vision.UserLibBuild import ensureVisionResourceFile
        ensureVisionResourceFile(resourceFolder=resourceFolder)
...
You will need to load / reload the library for the node to appear in it.
\n ------------------------------ \n
...
(the name of the library is hardcoded in the network file)
\n ------------------------------ \n
...
\n ------------------------------ \n
Please address questions about this mechanism to vareille@scripps.edu
""")
...
"""
Both Python versions 2.4 and 2.5 are supported.
...
Legal itemnames: 'All'
    ...
    def addButtonsToglyphButtonBar(self, master):
        glyphbuttonList = [ {'sep1':{'icon1':'sep.gif', 'icon2':None, 'state':'disabled',
                   ...
                   {'rectangle':{'icon1':'rectangle.gif', 'icon2':None, 'state':'normal',
                   ...
                   {'Underline':{'icon1':'underline.gif', 'icon2':None, 'state':'normal',
...
"""
    ...
    def stopCurrentNet_cb(self, event=None):
        ...
        bl['pause'].disable()
        bl['stop'].disable()
        self.currentNetwork.canvas.update_idletasks()
...
##          self.flowbt.pack(expand=1, fill='x')
    ...
    def addLibraryInstance(self, lib, modName, varName, force=False):
        """load and show library which is provided as an isntance of NodeLibary
...
"""
        NoGuiVPE.addLibraryInstance(self, lib, modName, varName, force)
...
you need to install: %s \n""" %(module,dep))
    ...
    def showLibrary(self, lib, force=False):
        ...
        if lib.name in self.ModulePages.pagenames():
            lib.page = self.ModulePages.delete(lib.name)
        lib.page = self.ModulePages.add(lib.name)
...
#            sc_canvas.resizescrollregion()
        ...
        Pmw.Color.changecolor(self.ModulePages.component(lib.name+'-tab'),
                              lib.color)
        ...
        self.ModulePages.selectpage(lib.name)
        lib.resizeCategories()
...
onto the network editor canvas."""
        ...
        sc_canvas = kbScrolledCanvas(
            ...
            usehullsize=1, hull_width=100, hull_height=150,
