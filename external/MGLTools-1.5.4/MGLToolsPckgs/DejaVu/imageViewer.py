...
try:
    ...
    warnings.warn('numarray not found')
    hasNumarray = False
...
#        average[1][1] = .0
    ...
    def secondDerivativeNum(self):
        ...
        a = self.numimarray
        c = a.copy()
        convolve.Convolve2d(sndDeriv, a, c)
        return c
    ...
    def firstDerivative(self):
        ...
        c = a.copy()
        convolve.Convolve2d(fstDeriveV, a, c)
        
        fstDeriveV = numarray.array([ [ 0.125,   0.25,  0.125],
                                     [ 0.0  ,    0.0,  0.0  ],
                                     [-0.125,  -0.25, -0.125] ], 'f')
        d = a.copy()
        convolve.Convolve2d(fstDeriveV, a, d)

        fstDeriveH = numarray.array([ [-0.125,    0.0, 0.125],
                                     [-0.25 ,    0.0, 0.25  ],
                                     [-0.125,    0.0, 0.125] ], 'f')
        e = a.copy()
        convolve.Convolve2d(fstDeriveH, a, e)

        fstDeriveH = numarray.array([ [ 0.125,    0.0, -0.125],
                                     [ 0.25 ,    0.0, -0.25  ],
                                     [ 0.125,    0.0, -0.125] ], 'f')
        f = a.copy()
        convolve.Convolve2d(fstDeriveH, a, f)
